...
| `engineering`          | `Member`      | Standard contributor inside Data Security.        |
...
4. The agent polls `/agent/identity/status` until the IdP confirms
   the binding.
5. The device row in **DLP → Endpoints** flips from `pending` to
...
- **Manually** by an admin from **DLP → Endpoints** → device
  row → **Force re‑bind**.
...
Mark these devices in **DLP → Endpoints** as `service` so they're
