...
(**GLiNER2‑PII**, Apache‑2.0, [fastino/gliner2-privacy-filter-PII-multi](https://huggingface.co/fastino/gliner2-privacy-filter-PII-multi))
...
Requires `runtime_security.view`. Also **warms the model**: the first
...
image with no `gliner2` package all just mean NER findings are absent
