...
| `403`  |,                                                 | Caller lacks the required permission scope. `detail` names it, e.g. `API key missing required scope: runtime:config:write`. | Reissue the key with that scope. See [API key scopes](/api-reference/introduction#api-key-scopes).    |
| `409`  |,                                                 | The action contradicts a decision already recorded, e.g. protecting a service somebody blocked.      | Read `detail`. It says what to do instead.                                                           |
